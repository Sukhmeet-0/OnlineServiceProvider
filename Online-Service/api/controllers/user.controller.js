...
async function AddProvider(req, res) {
    console.log(req.body)
    let { fullname, email, password, phone, city, starttime, endtime, slot, photo, categoryName, subCategory, address } = req.body
    let collection = 'providers'
    let document = await db.collection(collection).findOne({ email: email, fullname: fullname })
    if (document) {
        res.json({ error: 'User already exists', message: '' })
        // Qual.errordb('Error',res.error)
        ...
        try {
            let document = await db.collection(collection).insertOne({ name: fullname, email: email, password: password, phone: phone, city: city, address: address, starttime: starttime, endtime: endtime, price: slot, photo: photo, category: categoryName, subCategory: subCategory,status:"" })
            res.json({ error: '', message: "Provider added successfully" })
        } catch (err) {
            res.json({ error: err.message, message: "" })
        }
    }
}
async function ProviderSignIn(req,res){
    console.log(req.body)
    let { email, password } = req.body
    if (email == "" || password == "") {
        res.json({ error: 'All fields are required', message: '' })
